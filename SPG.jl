...
function spg(x0, f, ∇f, proj, ε, max_iter, lambda_min, lambda_max, M, sigma1, sigma2, η, linesearch)
    ...
    while true
        ...
        # Backtrackin routine
        (x, gradf_x, s, y, f_hist, alpha, et, evalf, evalproj) = linesearch(iter, lambda, x, gradf_x, f_hist, M, sigma1, sigma2, η, proj)
        push!(stplen,alpha)
        push!(feval,evalf)
...
function spg1(k,lambda_k,x_k,gradf_x_k,f_hist, M, sigma1, sigma2, η, proj)
    alpha = copy(lambda_k)
    t0 = time()
    evalf = 0
    evalproj = 0
...
#
function spg2(k,lambda_k,x_k,gradf_x_k,f_hist, M, sigma1, sigma2, η, proj)
